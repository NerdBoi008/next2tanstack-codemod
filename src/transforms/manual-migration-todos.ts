...
export const nextManualMigrationTodoTransform: SubTransform = async (root) => {
  ...
  const isRouteLikeFile =
    /[/\\](route|page|layout|loading|error|not-found|template)\.(ts|js|tsx|jsx)$/.test(
      filename,
    );
  const isClientComponent = hasTopLevelDirective(rootNode, "use client");
  const nextNavigationImports = findImportsBySource(
    rootNode,
    "next/navigation",
  );
  ...
  if (topLevelTodos.length > 0) {
    ...
    if (firstNode) {
      edits.push(
        firstNode.replace(`${topLevelTodos.join("\n")}\n${firstNode.text()}`),
      );
    }
  }
...
function findFirstNonImportIdentifier(
  rootNode: any,
  identifier: string,
): any | null {
...
function isMiddlewareOrEdgeRuntimeFile(
  rootNode: any,
  filename: string,
): boolean {
