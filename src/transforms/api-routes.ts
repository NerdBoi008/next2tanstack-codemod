...
${handlersCode}
  ...
  if (firstHandler) {
    ...
    for (let i = 1; i < methodHandlers.length; i += 1) {
      ...
      if (!handler) continue;
      edits.push(handler.node.replace(""));
    }
  }
...
function transformHandlerBody(
  ...
  // Transform request.json() - same in both
  // Transform request.url - same in both
...
export const nextDefaultExportApiTransform: SubTransform = async (root) => {
  const rootNode = root.root();
  const edits: Edit[] = [];
  const filename = root.filename();
