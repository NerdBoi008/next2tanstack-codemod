 ...
 * Handles: page.tsx -> route.tsx, layout.tsx -> __root.tsx, loading.tsx -> -pending.tsx
 * Dynamic routes: [id] -> $id, [...slug] -> $, [[...catchall]] -> $
 */
export const nextToTanstackFileStructureTransform: SubTransform = async (
  root,
) => {
  ...
  // Skip node_modules and non-app files
  if (
  ...
  // Transform 1: page.tsx -> route.tsx (or index.tsx if in directory)
...
${buildRouteComponent("NotFoundComponent", "", body, isAsync)}`;
  ...
  // Transform 6: template.tsx -> _layout.tsx (similar to layout but remounts)
  if (effectiveFilename.match(/template\.(tsx|jsx|ts|js)$/)) {
    ...
    if (templateExport) {
      tanstackImportSpecifiers.add("createFileRoute");
      tanstackImportSpecifiers.add("Outlet");
      const propsNode = templateExport.getMultipleMatches("PROPS");
      const bodyNode = templateExport.getMultipleMatches("BODY");
      const props = propsNode.map((p: any) => p.text()).join(", ");
      const body = bodyNode.map((b: any) => b.text()).join("\n");

      // Templates in TanStack can be handled with _layout and key changes
      const isAsync = templateExport.text().includes("async function");
      const templateRoute = `export const Route = createFileRoute('${calculateTanstackRoute(effectiveFilename)}')({
  component: TemplateComponent,
  // Note: Template remounts on navigation, use key prop if needed
})

${buildRouteComponent("TemplateComponent", props, body, isAsync)}`;

      edits.push(templateExport.replace(templateRoute));
    }
  }
 ...
 */
export const nextRouteGroupsTransform: SubTransform = async (root) => {
  const rootNode = root.root();
  const edits: Edit[] = [];
  const filename = root.filename();
  const tanstackImportSpecifiers = new Set<string>();
 ...
 */
export const nextApiRoutesTransform: SubTransform = async (root) => {
  const rootNode = root.root();
  const edits: Edit[] = [];
  const filename = root.filename();
  const tanstackImportSpecifiers = new Set<string>();
