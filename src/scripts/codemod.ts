...
function mapRouteFilename(base: string, isRootLayout: boolean): string | null {
  if (/^page\.(tsx|jsx|ts|js)$/.test(base))
    return base.replace(/^page\./, "route.");
  ...
  if (/^template\.(tsx|jsx|ts|js)$/.test(base))
    return base.replace(/^template\./, "_template.");
...
const transform: CodemodTransform = async (root, options?: unknown) => {
  ...
  if (!isDryRun() && targetPath) {
    ...
    await writeMovedFileAndDeleteOriginal(currentFilename, targetPath, output);
    return null;
  }
