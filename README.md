...
- Migration enable/disable behavior is controlled via `next-to-start.codemod.json` (or CLI/env overrides where applicable).
...
```sh
npx next2tanstack@latest [appDir] [flags]
```

e.g.

```sh
npx next2tanstack@latest ./app --dry-run --diff
```

Apply changes:

```sh
npx next2tanstack@latest ./app
```

Flags:

- `--dry-run`: preview changes without writing files.
- `--diff`: print per-file unified diff output (with file separators and red/green line coloring in TTY).
...
2. CLI/env (`--routes-directory`, `CODEMOD_ROUTES_DIRECTORY`, fallback `ROUTES_DIRECTORY`)
